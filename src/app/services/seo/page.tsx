...
export const metadata = pageMetadata("SEO Agency London | SEO Services", description, "/services/seo");
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.section} id="seo-work" aria-labelledby="work-title">
        <div className="container">
          <div className={styles.sharedHeading}><SectionHeading id="work-title" kicker="Selected work" title={<>Real businesses.<br /><span>Different search journeys.</span></>} intro="A restaurant booking. A stone restoration enquiry. A search for a specific car part. The website needs to match the way its customers search and choose." /></div>
          <article className={styles.featuredProject} data-reveal="project">
          ...
          <div className={styles.projectPair} data-reveal="projects">
      ...
      <section className={styles.perspective} aria-labelledby="perspective-title"><div className={`container ${styles.perspectiveGrid}`}><div><p className={styles.eyebrow}>Visibility is the beginning</p><h2 id="perspective-title">Being found matters.<br /><span>So does what happens next.</span></h2></div><div><p>Can visitors quickly understand your service, see evidence they trust and make a relevant enquiry? We examine that journey alongside your search performance.</p><p>The priority is a website that helps the right person take the next step.</p><Link className={styles.textLink} href="/services/web-conversion">Explore web &amp; conversion <Arrow diagonal /></Link><Link className={styles.textLink} href="/services/local-seo-london">Explore Local SEO London <Arrow diagonal /></Link><Link className={styles.textLink} href="/services/technical-seo-london">Explore Technical SEO London <Arrow diagonal /></Link></div></div></section>

      <section className={styles.section} id="seo-deliverables" aria-labelledby="deliverables-title"><div className="container"><div className={styles.sharedHeading}><SectionHeading id="deliverables-title" kicker="SEO services, made practical" title={<>Know what the work<br /><span>is meant to achieve.</span></>} intro="A clear plan is useful only when someone can act on it. We agree the priorities, deliverables and responsibilities for your project." /></div><div className={styles.deliverables}>{deliverables.map((item, index) => <article key={item.title}><span className={styles.rowNumber}>0{index + 1}</span><h3>{item.title}</h3><div><p>{item.copy}</p><p className={styles.output}><span>Output</span>{item.output}</p></div></article>)}</div></div></section>

      <section className={styles.approach} id="seo-approach" aria-labelledby="approach-title"><div className="container"><div className={styles.sharedHeading}><SectionHeading id="approach-title" kicker="Our approach" title={<>Start with a plan<br /><span>you can inspect.</span></>} intro="Your starting point shapes the work. We keep the plan practical, the responsibilities clear and the next decision grounded in evidence." /></div><ol className={styles.steps}>{steps.map((step, index) => <li key={step.title}><span className={styles.stepNumber}>0{index + 1}</span><h3>{step.title}</h3><p>{step.copy}</p></li>)}</ol></div></section>

      <section className={styles.section} aria-labelledby="founder-title"><div className={`container ${styles.founderGrid}`} data-reveal="founder"><div className={styles.founderImage}><SeoImageSlot asset={seoImages.founder} number="05" /></div><div className={styles.founderCopy}><p className={styles.eyebrow}>Founder-led, from the start</p><h2 id="founder-title">Strategy and execution.<br /><span>Part of the same conversation.</span></h2><p>WD Marketing is led by Wael, bringing web development, SEO and conversion thinking together around your business priorities.</p><p>Before work begins, you should know who is leading the project, who is making changes and what input is needed from you. Your proposal sets out those responsibilities alongside the scope.</p><div className={styles.founderSignature}><strong>Wael Daas</strong><span>Founder, WD Marketing</span></div><Link className={styles.textLink} href="/about">Meet the founder <Arrow diagonal /></Link></div></div></section>
