...
export default function CroPage() {
  return <>
    ...
    <div className={styles.page}>
      ...
      <section className={`${styles.section} ${styles.auditSection}`} id="audit" aria-labelledby="audit-title"><div className="container"><div className={styles.heading}><div><p>THE CRO AUDIT</p><h2 id="audit-title">We inspect the system around the conversion, not one isolated page element.</h2></div><p>The audit combines behavioural evidence, commercial intent and implementation reality. Start with our <Link href="/insights/landing-page-before-more-ad-spend">Google Ads landing page checklist</Link> to review message match, mobile forms and qualified enquiries.</p></div><div className={styles.auditGrid}>{auditAreas.map((item,index)=><article key={item.title}><span>0{index+1}</span><div><h3>{item.title}</h3><p>{item.copy}</p></div></article>)}</div></div></section>
