...
export function SelectedWork() {
  return <section id="work" className={styles.section} data-studio-section="work" aria-labelledby="work-heading"><div className="container">
    ...
    <div className={styles.list}>{projects.map((project,index)=><article key={project.slug} className={styles.project} data-studio-card data-reveal>
      ...
      <div className={styles.media} data-studio-project-media><MediaFrame asset={resolveMedia({src:project.image,alt:project.imageAlt,width:1800,height:1200})} label={project.name} fill sizes="(max-width: 820px) calc(100vw - 40px), (max-width: 1200px) 52vw, 680px"/><span className={styles.corner}>WD / Case study</span></div>
    </article>)}</div>
  </div></section>;
}
