...
export function Founder() {
  return <section id="about" className={styles.section} data-studio-section="founder" aria-labelledby="founder-heading"><div className="container">
    <div className={styles.frame} data-studio-founder-frame><FounderPortrait/><div className={styles.copy} data-reveal="copy">
    ...
    <div className={styles.principles} data-studio-grid>{principles.map(([number,title,copy]) => <article key={number} className={styles.principle} data-studio-card><span>{number}</span><h3>{title}</h3><p>{copy}</p></article>)}</div>
  </div></section>;
}
