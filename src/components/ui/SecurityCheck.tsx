...
export function SecurityCheck({ onToken, revision, siteKey = process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY }: { onToken: (token: string) => void; revision: number; siteKey?: string }) {
  ...
  useEffect(() => {
    ...
    // Compact mode also fits a 320px viewport without clipping the challenge.
    const id=api.render(target,{sitekey:siteKey,action:"project-brief",theme:"light",size:"compact",callback:(token:string)=>callback.current(token),"expired-callback":()=>callback.current(""),"error-callback":()=>{callback.current("");setFailed(true);}});
