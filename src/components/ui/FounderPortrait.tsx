...
export function FounderPortrait({ wide = false }: { wide?: boolean }) {
  ...
  return <div className={`${styles.frame} ${styles.identity}`} data-studio-identity><p className={styles.label}>Founder-led · London</p><span className={styles.monogram} aria-hidden="true">WD</span><div><p className={styles.name}>Wael</p><p className={styles.role}>Founder & Digital Growth Strategist</p><p className={styles.note}>Strategy, development and marketing — connected around the same business goal.</p></div></div>;
}
