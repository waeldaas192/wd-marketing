...
import { ArrowIcon, SendRocketIcon } from "./Icons";
...
export function ContactForm() {
  ...
  function next(){interacted.current=true;if(checkStep()){setStep(current=>Math.min(2,current+1));setFeedback("");setState("idle");}else{setFeedback("Check the highlighted fields before continuing.");setState("error");requestAnimationFrame(()=>formRef.current?.querySelector<HTMLElement>('[aria-invalid="true"]')?.focus());}}
  ...
  return <form ref={formRef} className="project-form" onSubmit={submit} noValidate aria-busy={sending} data-project-wizard>
    ...
    <fieldset className="form-block" hidden={step!==0} disabled={sending||state==="success"}><legend>What do you need?</legend><div className="choice-grid">{contactServices.map(item=><label key={item}><input type="radio" name="service" value={item} required checked={brief.service===item} onChange={()=>update("service",item)} aria-invalid={!!errors.service} aria-describedby={errors.service?"error-service":undefined}/><b>{item}</b></label>)}</div>{error("service")}<p className="form-budget-label">Indicative budget</p><div className="choice-grid">{contactBudgets.map(item=><label key={item}><input type="radio" name="budget" value={item} required checked={brief.budget===item} onChange={()=>update("budget",item)} aria-invalid={!!errors.budget} aria-describedby={errors.budget?"error-budget":undefined}/><b>{item}</b></label>)}</div>{error("budget")}</fieldset>
    <fieldset className="form-block" hidden={step!==1} disabled={sending||state==="success"}><legend>How can we reach you?</legend><div className="field-grid">{([{key:"name",label:"Full name",type:"text",auto:"name",max:150},{key:"email",label:"Email",type:"email",auto:"email",max:254},{key:"company",label:"Company (optional)",type:"text",auto:"organization",max:200},{key:"website",label:"Website (optional)",type:"url",auto:"url",max:2000}] as const).map(field=><div key={field.key}><label htmlFor={`project-${field.key}`}>{field.label}</label><input id={`project-${field.key}`} name={field.key} type={field.type} required={field.key==="name"||field.key==="email"} value={brief[field.key]} onChange={event=>update(field.key,event.target.value)} autoComplete={field.auto} maxLength={field.max} placeholder={field.key==="website"?"https://example.com":undefined} aria-invalid={!!errors[field.key]} aria-describedby={errors[field.key]?`error-${field.key}`:undefined}/>{error(field.key)}</div>)}</div></fieldset>
    <fieldset className="form-block" hidden={step!==2} disabled={sending||state==="success"}><legend>What should this project change?</legend><div className="textarea-label"><label htmlFor="project-message">Your goal</label><textarea id="project-message" name="message" required value={brief.message} onChange={event=>update("message",event.target.value)} rows={6} maxLength={5000} aria-invalid={!!errors.message} aria-describedby={errors.message?"error-message":"message-hint"}/>{error("message")}</div><p id="message-hint" className="form-note">Tell us the business goal, what is blocking it and any timeline. Please do not include passwords or sensitive personal information.</p><dl className="brief-summary"><div><dt>Service</dt><dd>{brief.service}</dd></div><div><dt>Budget</dt><dd>{brief.budget}</dd></div><div><dt>Reply to</dt><dd>{brief.email}</dd></div></dl>{step===2&&state!=="success"&&<SecurityCheck onToken={setToken} revision={revision}/>}</fieldset>
    {state!=="success"&&<div className="form-submit">{step>0&&<button type="button" className="button button-ghost" disabled={sending} onClick={()=>{interacted.current=true;setStep(value=>value-1);setState("idle");setFeedback("");}}><ArrowIcon direction="left"/> Back</button>}<button type="submit" className={`button button-primary liquid-cta${step===2?" submit-motion":""}${sending?" is-sending":""}`} disabled={sending} data-sending={sending?"true":undefined}>{step<2?<><span>Continue</span><ArrowIcon/></>:<><span>{sending?"Sending":"Submit project brief"}</span><SendRocketIcon active={sending}/></>}</button>{step===2&&<p>Read our <Link href="/privacy">privacy policy</Link> before submitting.</p>}</div>}
    {feedback&&<div className="form-status" role={state==="error"?"alert":"status"}>{feedback}</div>}
    {state==="success"&&<button type="button" className="button button-ghost" onClick={()=>{setBrief({...emptyBrief});setErrors({});setFeedback("");setState("idle");setCopied(false);interacted.current=true;setStep(0);lastRequest.current={signature:"",id:""};}}>Start another brief</button>}
    <div className="brief-alternative"><p>Prefer email? Copy your brief, then paste it into a message.</p><button type="button" className="button button-ghost" onClick={async()=>{try{await navigator.clipboard.writeText(plainBrief);setCopied(true);}catch{setFeedback("Copy is unavailable in this browser. Your entries remain in the form.");setState("error");}}}>{copied?"Brief copied":"Copy brief"}</button><a className="brief-email-link" href={mailto}>Email {site.email}<ArrowIcon/></a></div>
