...
export function Header() {
  ...
  useEffect(() => {
    ...
    }, paused || reduced ? 0 : MOBILE_MENU_REVEAL_DELAY);
    return () => {
      window.clearTimeout(reveal);
      document.body.style.overflow = previous;
      if (dialog.open) dialog.close();
    };
  }, [mobileOpen, paused, reduced]);
  useEffect(() => {
  ...
  return <>
    ...
    <dialog ref={dialogRef} id="mobile-navigation" aria-label="Mobile navigation" className={styles.dialog} onKeyDown={event => {
      ...
      <div className={styles.dialogBar}><strong>WD Marketing</strong><button type="button" className={`${styles.menu} ${styles.menuOpen} ${styles.dialogClose}`} aria-label="Close menu" onClick={() => { restoreMobileFocus.current = true; setMobileOpen(false); }}><span className={styles.menuGlyph} aria-hidden="true"><i/><i/></span></button></div>
