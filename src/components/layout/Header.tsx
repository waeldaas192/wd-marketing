      ...
      <header className={styles.header}>
        <div className={`container ${styles.inner}`}>
          ...
          <nav className={styles.nav} aria-label="Primary navigation">
            ...
            <button
              type="button"
              className={`${styles.link} ${styles.servicesTrigger} ${megaOpen ? styles.active : ""}`}
            ...
            {primaryLinks.slice(1).map((item) => (
              <Link key={item.href} href={item.href} className={styles.link} onClick={() => setMegaOpen(false)}>{item.label}</Link>
            ))}
          </nav>
        ...
        <div className={`container ${styles.mobileInner}`}>
          <div className={styles.mobileLinks}>
            ...
            <div className={styles.mobileServices}>
              <div className={styles.mobileServicesTitle}>Services <span>02</span></div>
              {serviceLinks.map((item) => <Link key={item.href} href={item.href} onClick={closeAll}>{item.label} <i>↗</i></Link>)}
            </div>
          ...
          <div className={styles.mobileBottom}>
            <p>London-based digital growth studio.</p>
            <Link href="/contact" className={styles.cta} onClick={closeAll}>Start a project <span>↗</span></Link>
          </div>
        </div>
      </div>
    </>
